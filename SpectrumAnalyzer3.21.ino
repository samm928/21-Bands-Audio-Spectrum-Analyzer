...
//Now let's set up the frequency board to output the needed frequencies
  ...
  FrequencyBoard.init(25000000L);                         // Remove MSGEQ7 ICs and use a 1K resistor from Pin8 to Pin3 to serial monitor in Debug Mode.

  FrequencyBoard.setFreq(0, 104570);                      // 104570 original clock F0
  FrequencyBoard.setFreq(1, 166280);                      // 166280 original F0 or calculated F1 = F0 x 1.3572 = 141.992
  FrequencyBoard.setFreq(2, 192617);                      // Calculated from original clock F0 to be F2 = F0 x 1.3572^2 = 192.617

  FrequencyBoard.setPower(0, SIOUT_8mA);
  FrequencyBoard.setPower(1, SIOUT_8mA);
  FrequencyBoard.setPower(2, SIOUT_8mA);

  FrequencyBoard.enable(0);
  FrequencyBoard.enable(1);
  FrequencyBoard.enable(2);
...
// READ IN MGSEQ7 VALUES  ***********************************************************************************************
   ...
   bandValues[i] = constrain(bandValues[i], 0, 1023);
